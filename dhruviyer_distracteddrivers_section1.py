# -*- coding: utf-8 -*-
"""DhruvIyer_DistractedDrivers_Section1.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1nXm0YEewShMewHmx8JOu3MEnG-DuqSII
...
train_data, train_labels = get_train_data() #(length, width, height, color)
image = train_data[0,:]
...
for x in range(1, 50):
  ...
  list.append(accuracy_score(test_labels, y_pred))
max = list[0]
for x in list:
  if x > max:
    max = x
print(max, '----', list.index(max))
...
(test_data, test_labels) = get_test_data(flatten = True)
for x in range(1, 50):
  dModel = DecisionTreeClassifier(max_depth = x)
  dModel.fit(train_data, train_labels)
  y_pred = dModel.predict(test_data)
  dlist.append(accuracy_score(test_labels, y_pred))
max = dlist[0]
for x in dlist:
  if x > max:
    max = x
print(max, '----', dlist.index(max))
