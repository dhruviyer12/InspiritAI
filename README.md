...
This project was built as part of an Inspirit AI program to figure out if a driver is paying attention to the road or getting distracted. It looks at pictures from a dashboard camera and puts the driver into one of four groups: paying attention, drinking coffee, checking the mirror, or messing with the radio. The code walks through the whole process, starting with basic neural networks and moving all the way up to advanced image models and transfer learning. It also includes tools to test how well the models work and to see how they make their choices.
...
First, the code imports tools like TensorFlow and changes text labels into numbers. It tweaks the images by flipping and zooming them so the model learns actual shapes instead of memorizing exact photos. It also reads a spreadsheet to organize the pictures, making sure every driving action has an equal amount of data. Finally, it splits the pictures into a training pile and a testing pile.

### Part 2: Building the Models

Next, the code creates helper functions to draw graphs and display pictures. Then it builds three different models to see which works best. The first is a basic model that flattens the picture into a list of pixels. The second is a custom convolutional model that sweeps over the picture in grids to find patterns.


The third uses transfer learning, borrowing a pre-trained model called ```VGG16``` to specifically recognize distracted driving.

### Part 3: Testing and Looking Inside

Finally, the code trains the models and grades them using the test pictures. It builds a confusion matrix to show exactly where the models messed up.  At the end, it runs a tool to create heatmaps on the pictures.  These heatmaps light up the exact spots that made the model pick its answer, proving it is actually looking at the driver.
